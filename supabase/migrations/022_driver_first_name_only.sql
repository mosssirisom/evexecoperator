-- ─── Use the driver's first name only in the customer 24-hour reminder ────────
-- Only change from 021: the reminder now says "Your driver, James, will be in
-- a …" rather than the full name. Everything else is identical.
...
begin
  ...
  if v_did is not null then
    ...
    v_dfirst := split_part(trim(coalesce(v_dname, '')), ' ', 1);  -- first name only

-- ─── Fare on the customer 24-hour reminder (gated by payment status) ──────────
-- The customer's 24h reminder now tells them about the fare:
--   Unpaid   → "Please have £150.00 ready for your driver."
--   Invoiced → "Your fare will be invoiced."
--   Paid     → "Your fare has been paid in advance — nothing to pay the driver."
--   (no price on file → no fare line)
-- The confirmation ("booked") text and the status-update texts are unchanged.
--
-- The reminder is queued when the booking is created, so a later change to the
-- payment status rewrites the pending reminder to stay accurate at send time.
-- Only affects operator bookings (source='operator'); website bookings get
-- their own customer messaging from the public site.
...
begin
  ...
  -- Customer-facing fare clause for the reminder.
  ...
  if tg_op = 'INSERT' then
    ...
    -- Confirmation ("booked") — unchanged, no fare line.
    ...
    -- 24-hour reminder, now with the fare clause.
    ...
    -- Keep the pending reminder's fare line accurate when payment status changes.

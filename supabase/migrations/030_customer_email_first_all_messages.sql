-- Customers: email with SMS fallback for ALL messages. Previously the
-- "driver on the way / arrived" status updates went SMS-first; now every
-- customer message prefers email and only falls back to SMS when there's no
-- email address. Only change from the previous function body is that the
-- status_update channel pick uses notif_pick_channel(false, ...) instead of the
-- urgent flag.
...
declare
  ...
  v_dclause text := '';
  v_reminder text; v_msg text; v_conf text; v_ch text; v_urgent boolean;
begin
  ...
  if tg_op = 'INSERT' then
    ...
    v_ch := notif_pick_channel(false, v_email is not null, v_phone is not null);
    if v_ch is not null
       and not exists (select 1 from notification_queue q where q.booking_id=new.id and q.type='received')
    ...
    if v_sched is not null and v_sched - interval '24 hours' > now() and v_reminder is not null then
      v_ch := notif_pick_channel(false, v_email is not null, v_phone is not null);
      if v_ch is not null and not exists (select 1 from notification_queue q where q.booking_id=new.id and q.type='reminder_24h') then
        if v_ch='email' then
          insert into notification_queue (id,booking_id,type,channel,recipient,subject,body,html,status,attempts,next_attempt_at,created_at)
    ...
    if new.status is distinct from old.status then
      if new.status='En Route' and old.status='Dispatched' then v_msg := 'EV Exec: your driver is on the way. Ref '||new.ref||'.'; v_urgent := true;
      elsif new.status='Arrived' and old.status='En Route' then v_msg := 'EV Exec: your driver has arrived at the pickup point. Ref '||new.ref||'.'; v_urgent := true;
      elsif new.status='Cancelled' and old.status<>'Completed' then v_msg := 'EV Exec: your booking '||new.ref||' has been cancelled. Please contact us if this is unexpected.'; v_urgent := false;

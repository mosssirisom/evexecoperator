-- ─── Don't text the customer when voiding an already-Completed job ────────────
-- Cancelling a booking normally sends a "your booking has been cancelled" SMS.
-- But cancelling a job that was already Completed is an internal correction /
-- void, not a real cancellation, so the customer shouldn't be alarmed. Suppress
-- the status_update SMS specifically for the Completed -> Cancelled transition;
-- every other status text is unchanged. Applied to live Supabase and verified
-- (Completed->Cancelled enqueues 0 texts; a normal En Route->Cancelled still
-- enqueues 1).
...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    -- Voiding an already-completed job is an internal correction; don't alarm
    -- the customer with a cancellation text.

...
function Dashboard({ onSignOut }: { onSignOut: () => void }) {
  ...
  const filteredTransfers = useMemo(() => {
    ...
    return upcomingBookings.filter((b) => {
      ...
      if (!q) return true;
      const haystack = [b.ref, b.customer_name, b.customer_phone, b.customer_email, b.airport, b.dropoff_address, b.flight_number, b.drivers?.full_name]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
    ...
    if (driverId) {
      ...
      if (booking?.travel_date) {
        if (unavailableDriverIds(booking.travel_date).has(driverId)) {
          const driverName = drivers.find((d) => d.id === driverId)?.full_name ?? "This driver";

...
// `embedded` renders the calendar inside another shell (the operator app),
// hiding its own branded header and full-screen chrome so only one header shows.
export function CalendarApp({ embedded = false }: { embedded?: boolean }) {
  const { session, loading: authLoading, signIn, signOut } = useAuth();

  if (authLoading) {
    return (
      <div className={`${embedded ? "" : "min-h-screen bg-[#E9EBF2]"} flex items-center justify-center py-12 text-slate-600 text-sm gap-2`}>
  ...
  return <Dashboard onSignOut={signOut} embedded={embedded} />;
}
...
function Dashboard({ onSignOut, embedded = false }: { onSignOut: () => void; embedded?: boolean }) {
  ...
  return (
    <div className={`${embedded ? "" : "min-h-screen bg-[#E9EBF2]"} flex flex-col`}>
      ...
      {!loading && (
        <main className={embedded ? "flex-1" : "flex-1 overflow-y-auto"}>
